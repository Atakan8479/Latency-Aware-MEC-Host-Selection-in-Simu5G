...
void MecOrchestrator::initialize(int stage)
{
    ...
    if (stage != inet::INITSTAGE_LOCAL){
        ...
        scheduleAt(simTime() + 1, throughputTimer);
        return;
    }
    ...
    taskDelaySignal = registerSignal("TaskDelay_ms");
    selectedLatencySignal = registerSignal("SelectedHostLatency_ms");
    throughputSignal = registerSignal("throughput");
    packetLossSignal = registerSignal("packetLoss");
    jitterSignal = registerSignal("jitter");
    rttSignal = registerSignal("rtt");
    radioUtilizationSignal = registerSignal("radioUtilization");
    cpuUtilizationSignal = registerSignal("cpuUtilization");
    energySignal = registerSignal("EnergyUsed_mJ");
    mecCpuLoadSignal = registerSignal("MecCpuLoad");


    simtime_t processingTime = SIMTIME_ZERO; // or set actual processing delay if available
...
void MecOrchestrator::handleMessage(cMessage *msg)
{
    ...
    if (msg == throughputTimer) {
        // Placeholder for bitsSent: replace with actual bits sent since last check
        double bitsSent = 0;  // You need to compute or fetch this from relevant modules
        emit(throughputSignal, bitsSent / simtime_t::parse("1s").dbl());

        scheduleAt(simTime() + 1, throughputTimer);  // reschedule
        return;  // Don't delete self-messages!
    }
    delete msg;
}
...
void MecOrchestrator::startMECApp(UALCMPMessage *msg)
{
    ...
    emit(packetLossSignal, 1);  // Count as one failed deployment = packet/context loss
    ...
    if (bestHost != nullptr) {
        simtime_t measuredRTT = SimTime::parse("5ms");  // Replace with actual measurement logic if available
        emit(rttSignal, measuredRTT.dbl());

        double utilizationValue = 65.0;  // example percentage, simulate or get from bestHost
        emit(radioUtilizationSignal, utilizationValue);

        double cpuLoadValue = 72.0;  // simulated; replace with dynamic value from VIM if available
        emit(cpuUtilizationSignal, cpuLoadValue);

        double currentCpuLoad = cpuLoadValue;  // Use separate logic if available
        emit(mecCpuLoadSignal, currentCpuLoad);

        double energyConsumed_mJ = 20.0;  // Replace with dynamic value if available
        emit(energySignal, energyConsumed_mJ);

        simtime_t selectedLatency = par("latencyHost1");  // ideally should be dynamically measured
        if (selectedLatency != SIMTIME_ZERO && selectedLatency != SIMTIME_MAX) {
            EV << "Emitting selectedLatency: " << selectedLatency.inUnit(SIMTIME_MS) << " ms\n";
            emit(selectedLatencySignal, selectedLatency.inUnit(SIMTIME_MS));
        } else {
            EV << "SelectedLatency not available or invalid\n";
            emit(selectedLatencySignal, 0);  // Fallback value to ensure CSV alignment
        }
        ...
        if (!appInfo->status) {
            ...
            emit(packetLossSignal, 1);  // Consider it a loss if MEC instantiation failed
        ...
        msg->setSuccess(true);
         ...
         simtime_t delay = processingTime + instantiationTime;
         if (delay != SIMTIME_ZERO && delay != SIMTIME_MAX) {
             EV << "Emitting taskDelay: " << delay.inUnit(SIMTIME_MS) << " ms\n";
             emit(taskDelaySignal, delay.inUnit(SIMTIME_MS));
         } else {
             EV << "TaskDelay not available or invalid\n";
             emit(taskDelaySignal, 0);  // Fallback
         }

         simtime_t currentDelay = delay;
         emit(jitterSignal, fabs((currentDelay - lastDelay).dbl()));
...
cModule *MecOrchestrator::findBestMecHost(const ApplicationDescriptor& appDesc)
{
    ...
    for (auto mecHost : mecHosts) {
            ...
            double currentCpuLoad = uniform(20.0, 80.0);  // simulate CPU load between 20% and 80%
            emit(mecCpuLoadSignal, currentCpuLoad);
            ...
            if (!res) {
                EV << "MecOrchestrator::findBestMecHost - MEC host [" << mecHost->getName()
                   << "] has not got enough resources. Searching again..." << endl;
                continue;
            }

            // -------- Get and check mecPlatformManager submodule --------
            ...
            if (appDesc.getAppServicesRequired().empty()) {
                EV << "MecOrchestrator::findBestMecHost - App requires no services, selecting this host: "
                   << mecHost->getName() << endl;
                bestHost = mecHost;
                break;
            }

            std::string serviceName = appDesc.getAppServicesRequired()[0];
            for (const auto& service : *mecServices) {
                if (serviceName == service.getName()) {
                    EV << "MecOrchestrator::findBestMecHost - MEC host [" << mecHost->getName()
                       << "] provides required service: " << serviceName << endl;
                    bestHost = mecHost;
                    break;
                }
            }
        ...
        if (bestHost != nullptr)
            ...
            EV << "MecOrchestrator::findBestMecHost - no suitable MEC host found" << endl;
